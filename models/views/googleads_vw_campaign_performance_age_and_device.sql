...
with campaign_performance_by_age_range_and_device as (
    select * from {{ source('googleads_source', 'campaign_performance_by_age_range_and_device') }}
),
final as (
    select
    ...
    from campaign_performance_by_age_range_and_device
)
select * from final